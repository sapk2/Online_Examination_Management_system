@extends('layouts.app')
@section('content')
<h1 class="text-3xl font-bold px-4 py-2">Edit Faculty</h1>
<div class="grid-layout drop-shadow-md px-4 min-h-min">
    ...
    <form method="post" action="{{ route('faculties.update', ['id' => $faculty->id]) }}">
        ...
        <div class="mb-4">
            ...
            <input type="radio" name="status" id="status" value="1" <?php if ($faculty->status == "1") {
                                                                        echo 'checked';
                                                                    } ?>> Enable &nbsp;
            <input type="radio" name="status" id="status" value="0" <?php if ($faculty->status != "1") {
                                                                        echo 'checked';
                                                                    } ?>> Disable
        </div>
